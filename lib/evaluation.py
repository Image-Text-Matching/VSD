...
def encode_data(model, data_loader, log_step=10, logging=logger.info):
    ...
    for i, data_i in enumerate(data_loader):
        ...
        images, image_lengths, captions, teacher_aux_captions, teacher_captions, lengths, ids, img_ids, repeat = data_i
        model.logger = val_logger

        # compute the embeddings
        img_emb, cap_emb = model.forward_emb(images, captions, teacher_aux_captions, teacher_captions, img_ids, lengths,
...
def encode_data_bge(model, data_loader, log_step=10, logging=logger.info):
    batch_time = AverageMeter()
    val_logger = LogCollector()
    ...
    for i, data_i in enumerate(data_loader):
        ...
        # compute the embeddings
        img_emb, cap_emb = teacher_aux_captions, teacher_captions
...
def evalrank(model_path, opt=None, tokenizer=None, model=None, split='dev', fold5=False, save_path=None,
             data_path=None):
...
def evalrank_cocoxf30k(model_path, opt=None, tokenizer=None, model=None, split='dev', fold5=False, save_path=None,
             data_path=None):
    # load model and options
    checkpoint = torch.load(model_path, map_location='cuda')
    ...
    with torch.no_grad():
        img_embs, cap_embs = encode_data(model, data_loader)
        # img_embs, cap_embs = encode_data_zh(model, data_loader)
...
def evalrank_bge(model_path, opt=None, tokenizer=None, model=None, split='dev', fold5=False, save_path=None,
                 data_path=None):
    # load model and options
    checkpoint = torch.load(model_path, map_location='cuda')
    ...
    logger.info('Computing embeddings.')
    with torch.no_grad():
        img_embs, cap_embs = encode_data_bge(model, data_loader)
